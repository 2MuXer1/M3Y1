...
@app.route("/")
def random_fact():
    return f'<h1>Вот здесь ты можешь посмотреть рандомный факт!</h1><a href="/random_fact">Посмотреть случайный факт!</a>'

@app.route("/random_fact")
def second():
    return f'<h1>{random.choice(facts_list)}\r\nА вот здесь можешь подбросить монетку!</h1><a href="/money_toss">Что тебе выпало?</a>'

@app.route("/money_toss")
def third():
    return f'<h1>{random.choice(moneytoss)}</h1>'
